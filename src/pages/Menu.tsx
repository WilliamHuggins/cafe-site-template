        ...
        <div className="flex flex-col lg:flex-row gap-16">
          ...
          <div className="lg:w-1/4">
            <div className="sticky top-32 flex flex-row lg:flex-col gap-4 overflow-x-auto lg:overflow-visible pb-4 lg:pb-0 scrollbar-hide">
              {categories.map((category) => (
                ...
                >
                  {translateOrFallback(t, `menuData.categories.${toKey(category)}.title`, category)}
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="lg:w-3/4 space-y-24">
            {Object.entries(menuData).map(([category, data]) => (
              ...
              >
                <div className="mb-10">
                  <h2 className="text-3xl font-bold tracking-tight text-black dark:text-white mb-4">{translateOrFallback(t, `menuData.categories.${toKey(category)}.title`, category)}</h2>
                  {data.notes.length > 0 && (
                    <div className="flex flex-wrap gap-x-6 gap-y-2 text-sm text-muted-text dark:text-dark-muted-text">
                      {data.notes.map((note, idx) => (
                        <span key={idx} className="flex items-center gap-2">
                          ...
                          {translateOrFallback(t, `menuData.categories.${toKey(category)}.notes.${idx}`, note)}
                        </span>
                      ))}
                    </div>
                  )}
                    ...
                    <div key={idx} className="group">
                      ...
                      {item.description && (
                        <p className="text-sm text-secondary-text dark:text-dark-secondary-text leading-relaxed pr-8">
                          {translateOrFallback(t, `menuData.categories.${toKey(category)}.items.${idx}.description`, item.description)}
                        </p>
                      )}
                    </div>
                  ))}
